...
export default function LegacySection() {
  ...
  useEffect(() => {
    const fetchLegacy = async () => {
      ...
      if (snap.exists()) setLegacyData(snap.data().legacy);
    };
    fetchLegacy();
  }, []);
  ...
  const steps = legacyData?.steps || defaultSteps;

  return (
    <section ref={sectionRef} className="relative py-24 md:py-16 bg-white overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row items-stretch md:rounded-xl overflow-hidden shadow-2xl border border-slate-100 min-h-[600px] lg:h-[70vh]">
          ...
          <div className="h-[350px] lg:h-auto lg:w-1/2 relative bg-slate-900 overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-transparent to-transparent lg:bg-gradient-to-r lg:from-blue-900/80 lg:via-transparent lg:to-transparent flex items-end p-6 lg:p-12">
              <motion.h3 initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} className="text-white text-xl lg:text-3xl font-serif italic max-w-xs md:max-w-sm">
                "{legacyData?.quote || "A legacy that lives on in the hearts of many."}"
              </motion.h3>
            </div>
          </div>
          ...
          <div className="lg:w-1/2 bg-slate-50 p-6 md:p-8 lg:p-12 flex flex-col justify-center">
            ...
            <div className="space-y-5 md:space-y-8">
              {steps.map((item: any, i: number) => (
                <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="grid grid-cols-[40px_1fr] md:grid-cols-[50px_1fr] items-start gap-3 md:gap-4">
                  <span className="text-3xl md:text-4xl font-black text-blue-200 leading-none">{item.step}</span>
                  <div>
