...
export default function HeroSection() {
    ...
    const WHATSAPP_NUMBER = "2347065870898"; 
    const WHATSAPP_MESSAGE = "Hello Laboc Funeral Services, I would like to make an inquiry regarding your services.";
    const HERO_IMAGE = "/services1.jpeg"
    ...
    useEffect(() => {
        const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
            if (user && user.uid === adminUID) {
                setIsAdmin(true);
                // Real-time listener for unread status
        ...
        <section className="relative min-h-[80vh] flex items-center justify-center bg-slate-950 overflow-visible">
            ...
            <div className="container relative z-20 mx-auto px-6 text-center overflow-visible">
                ...
                <h1 className="text-4xl md:text-7xl font-serif font-bold mb-6 tracking-tight text-white">
                    Compassionate <span className="text-blue-400">Funeral Services</span>
                </h1>

                <p className="text-lg md:text-xl mb-12 max-w-2xl mx-auto text-slate-300 leading-relaxed font-light">
                    At <span className="text-white font-medium">Laboc</span> Services, we provide dignified and personalized arrangements, honoring your loved ones with care, respect, and utmost professionalism.
                </p>
            ...
            {isAdmin && (
                <AdminInbox 
                    isOpen={isAdminOpen}
                    onClose={() => setIsAdminOpen(false)} // 👈 FIXED: Removed setUnreadCount(0)
                />
            )}
        </section>
    );
